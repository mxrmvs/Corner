...
const FOCUS_OPTIONS: { value: TrainingFocus; label: string; desc: string; risk: string; riskColor: string }[] = [
  ...
  { value: 'MOTIVATIONAL',label: 'Motivacional', desc: 'Aumenta moral bastante',   risk: 'Baixo',     riskColor: '#3B6D11' },
        ...
        {([
          ...
          <button key={value} onClick={() => setSub(value)}
            style={{
              ...
              borderBottom: sub === value ? '2px solid #E8432D' : '2px solid transparent',
      ...
      {sub === 'escalacao' && (
        <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: '20px' }}>
          <div>
            ...
            <FieldView players={myPlayers} formation={club.tactical.formation} />
          </div>
          <div>
            <p style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#6B6560', marginBottom: '8px' }}>
              Elenco por posição
                      ...
                      return (
                        <div key={p.id} style={{
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', width: '60px' }}>
                            {([[p.condition.stamina,'#4A7C59'],[p.condition.matchFitness,'#C9A84C'],[p.condition.morale,'#E8432D']] as [number,string][]).map(([v, c], idx) => (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '6px' }}>
                {FOCUS_OPTIONS.map(({ value, label, desc, risk, riskColor }) => (
                  <button key={value} onClick={() => setGlobal(value)}
                    style={{
                      padding: '10px 6px', border: `1px solid ${globalFocus === value ? '#1A1A1A' : '#D6CFC4'}`,
                      background: globalFocus === value ? '#1A1A1A' : 'white',
                      cursor: 'pointer', textAlign: 'center',
                    }}>
                    <div style={{ fontFamily: 'system-ui', fontSize: '11px', fontWeight: 700, color: globalFocus === value ? 'white' : '#1A1A1A', marginBottom: '3px' }}>{label}</div>
                    <div style={{ fontSize: '9px', color: globalFocus === value ? 'rgba(255,255,255,0.6)' : '#9E9890', marginBottom: '3px' }}>{desc}</div>
              ...
              return (
                <div key={p.id} style={{ background: 'white', border: '1px solid #D6CFC4', padding: '12px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '3px', marginBottom: '10px' }}>
                    {([['STA', p.condition.stamina,'#4A7C59'],['RIT', p.condition.matchFitness,'#C9A84C'],['MOR', p.condition.morale,'#E8432D']] as [string,number,string][]).map(([l, v, c]) => (
                      <div key={l} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <span style={{ fontSize: '8px', color: '#9E9890', width: '20px' }}>{l}</span>
                        <div style={{ flex: 1, height: '3px', background: '#E8E4DC' }}>
                          <div style={{ height: '3px', width: `${v}%`, background: c }} />
                        </div>
                  ...
                  {individual ? (
                    <select value={playerFocus[p.id] ?? 'TECHNICAL'}
                      onChange={e => setPlayerFocus(prev => ({ ...prev, [p.id]: e.target.value as TrainingFocus }))}
                      style={{ width: '100%', padding: '4px 6px', border: '1px solid #D6CFC4', fontFamily: 'system-ui', fontSize: '10px', color: '#1A1A1A', background: 'white', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', maxWidth: '600px' }}>
          <div style={{ background: 'white', border: '1px solid #D6CFC4', overflow: 'hidden' }}>
            ...
            {[
              ...
              <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 16px', borderBottom: '1px solid #F0EDE8', background: i % 2 === 0 ? 'white' : '#FAFAF8' }}>
